...
setInterval(() => {
  ...
  const query = 'SELECT * FROM [DB_MID_MESSAGING].[dbo].[tbl_t_send_whatsapp_groups] where status = 0';
  ...
  runQuery(query, result => {
    ...
    for (let i = 0; i < result.recordset.length; i++) {
      ...
      formData.append('id_group', result.recordset[i].id_group);
      formData.append('message', result.recordset[i].message);
      ...
      .then(response => {
        ...
        const updateQuery = `UPDATE [DB_MID_MESSAGING].[dbo].[tbl_t_send_whatsapp_groups] SET status = 1, remarks = 'success' WHERE id = '${result.recordset[i]['id']}'`;
      ...
      .catch(error => {
        ...
        const updateQuery = `UPDATE [DB_MID_MESSAGING].[dbo].[tbl_t_send_whatsapp_groups] SET status = 2, remarks = '${error.message.replace(/'/g, "''")}' WHERE id = '${result.recordset[i]['id']}'`;
        runQuery(updateQuery, updateResult => {});
      });
    }
  });
